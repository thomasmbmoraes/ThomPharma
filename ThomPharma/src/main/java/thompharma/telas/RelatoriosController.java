...
public class RelatoriosController {
    ...
    private void gerarClientes() {
        ...
        // intervalo em dias para filtro (null = todo o período)
        Integer dias = null;
        switch (periodo) {
            case "Últimos 30 dias":  dias = 30;   break;
            case "Últimos 90 dias":  dias = 90;   break;
            case "Últimos 12 meses": dias = 365;  break;
            default: break;
        }

        // Subquery para status mais frequente do cliente no período
        String filtroSubquery = dias != null
            ? "AND p2.data_pedido >= CURRENT_DATE - (? * INTERVAL '1 day')" : "";
        String filtroPrincipal = dias != null
            ? "WHERE p.data_pedido >= CURRENT_DATE - (? * INTERVAL '1 day')" : "";

        String sql =
            "SELECT c.nome, COUNT(p.id) AS total, MAX(p.data_pedido) AS ultimo, " +
            "    (SELECT p2.status FROM tb_pedidos p2 " +
            "     WHERE p2.id_cliente = c.id " + filtroSubquery +
        ...
        try (Connection conn = Conexao.conectar();
            ...
            int idx = 1;
            if (dias != null) {
                stmt.setInt(idx++, dias); // subquery
                stmt.setInt(idx++, dias); // cláusula principal
            }
            ResultSet rs = stmt.executeQuery();
            int pos = 1;
            while (rs.next()) {
