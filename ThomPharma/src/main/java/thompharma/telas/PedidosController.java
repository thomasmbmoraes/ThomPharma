...
public class PedidosController {
    ...
    private void carregarCombosAuxiliares() {
        try (Connection conn = Conexao.conectar()) {
            ...
            try (Statement stC = conn.createStatement();
                 ResultSet rsC = stC.executeQuery("SELECT id, nome FROM tb_clientes ORDER BY nome")) {
            ...
            try (Statement stP = conn.createStatement();
                 ResultSet rsP = stP.executeQuery("SELECT id, nome FROM tb_prescritores ORDER BY nome")) {
            ...
            // receitas
            try (Statement stR = conn.createStatement();
                 ResultSet rsR = stR.executeQuery("SELECT id, nome FROM tb_receitas ORDER BY nome")) {
            ...
            try (Statement stMp = conn.createStatement();
                 ResultSet rsMp = stMp.executeQuery(
            ...
            UiUtil.erro(mensagem, "Erro ao carregar dados: " + e.getMessage());
        }
    }
    ...
    private void salvar() {
        ...
        try (Connection conn = Conexao.conectar()) {
            if (idPedidoAtual == -1) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        if (rs.next()) idPedidoAtual = rs.getInt(1);
                    }
                }
                ...
                UiUtil.sucesso(mensagem, "Pedido criado com sucesso.");
                ...
                UiUtil.sucesso(mensagem, "Pedido atualizado com sucesso.");
            }
            ...
            UiUtil.erro(mensagem, "Erro ao salvar pedido: " + e.getMessage());
        }
    }
