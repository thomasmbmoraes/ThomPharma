...
public class RotulosController {
    ...
    private void salvar() {
        ...
        try (Connection conn = Conexao.conectar();
            ...
            mensagem.setText("Rótulo salvo com sucesso.");
            carregarRotulos();
    ...
    private void excluir() {
        ...
        try (Connection conn = Conexao.conectar();
            ...
            carregarRotulos();
            mensagem.setStyle("-fx-text-fill: green;");
            mensagem.setText("Rótulo excluído.");
        } catch (Exception e) {
            mensagem.setStyle("-fx-text-fill: red;");
    ...
    private void imprimir() {
        ...
        if (sucesso) {
            job.endJob();
            mensagem.setStyle("-fx-text-fill: green;");
            mensagem.setText("Rótulo enviado para impressão.");
        } else {
            mensagem.setStyle("-fx-text-fill: red;");
            mensagem.setText("Erro ao imprimir.");
        }
    }
    ...
    public static void gerarSeNaoExistir(int idPedido, String nomeFormula,
        ...
        try (Connection conn = Conexao.conectar()) {
            ...
            if (check.executeQuery().next()) return; // ja existe

            // calcula a proxima sequencia do dia
            PreparedStatement seq = conn.prepareStatement(
                "SELECT COALESCE(MAX(sequencia), 0) + 1 FROM tb_rotulos WHERE data_rotulo = CURRENT_DATE");
            ResultSet rsSeq = seq.executeQuery();
            int sequencia = rsSeq.next() ? rsSeq.getInt(1) : 1;
            ...
            System.out.println("Aviso: nao foi possivel gerar rotulo automatico: " + e.getMessage());
        }
    }
